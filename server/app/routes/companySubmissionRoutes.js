...
export default function submissionRoutes(db) {
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        const founder_uuid = req.user.uuid;
        ...
        const logoUrl = logo
          ? await uploadImageToR2(logo, `company-assets/${customId}/logo`)
          : null;

        const bannerUrl = banner
          ? await uploadImageToR2(banner, `company-assets/${customId}/banner`)
          : null;
        ...
        for (let i = 0; i < gallery.length; i++) {
          const url = await uploadImageToR2(
            gallery[i],
            `company-assets/${customId}/gallery/${i}`
          );
  ...
  // Global error handler for multer-specific issues
